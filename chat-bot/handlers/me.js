...
bot.command("myspace", async (ctx) => {
  if (ctx.from.id !== myId) return ctx.reply("🚫 *Unauthorized access.*", { parse_mode: "Markdown" });
...
bot.action("admin_broadcast", async (ctx) => {
  ...
  await ctx.reply("✉️ Please send the *message* you want to broadcast to all users.", { parse_mode: "Markdown" });
});
